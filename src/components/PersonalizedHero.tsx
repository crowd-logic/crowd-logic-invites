...
export const PersonalizedHero = ({ solution }: PersonalizedHeroProps) => {
  ...
  if (!solution) {
    return (
      <div id="personalized-hero" className="flex flex-col md:flex-row min-h-[90vh]">
  ...
  // Personalized state when solution is provided
  return (
    <div id="personalized-hero" className="flex flex-col md:flex-row min-h-[90vh]">
      {/* Context Side - Brand Ecosystem */}
      <div className="w-full md:w-1/2 flex flex-col justify-center items-center p-8 text-center text-white relative">
        <div className="absolute inset-0 bg-gradient-to-br from-slate-900 via-emerald-900 to-slate-900"></div>
        
        <div className="relative z-10 max-w-md">
          <div className="w-16 h-16 mb-6 mx-auto bg-white/10 rounded-full flex items-center justify-center backdrop-blur-sm">
          ...
          <p className="text-xl lg:text-2xl mb-8 text-white/90 leading-relaxed">
            Every solution in our platform works together seamlessly, creating powerful synergies 
            that amplify your success across the entire experiential marketing spectrum.
          </p>
        </div>
      </div>

      {/* Solution Side - Personalized Content */}
      <div className="w-full md:w-1/2 flex flex-col justify-center items-center p-8 text-center text-white relative border-l border-gray-700 min-h-[90vh]">
        <div className="absolute inset-0 bg-gradient-to-br from-indigo-900 via-purple-900 to-slate-900">
          {/* Animated particles or patterns */}
          <div className="absolute inset-0">
            <div className="absolute top-1/3 left-1/3 w-2 h-2 bg-white/30 rounded-full animate-pulse"></div>
            <div className="absolute top-2/3 right-1/3 w-3 h-3 bg-white/20 rounded-full animate-pulse delay-500"></div>
            <div className="absolute top-1/2 left-1/4 w-1 h-1 bg-white/40 rounded-full animate-pulse delay-1000"></div>
          </div>
        </div>
        
        <div className="relative z-10 max-w-md">
          <BarChart3 className="w-12 h-12 mb-6 mx-auto text-purple-300" />
          <h1 className="text-4xl lg:text-5xl font-bold mb-6 leading-tight">
            {solution.title || "Your Perfect Solution"}
          </h1>
          <p className="text-xl lg:text-2xl mb-8 text-purple-200 leading-relaxed">
            {solution.description || "We've identified the ideal CrowdLogic solution for your specific needs."}
          </p>
          <button className="group bg-purple-600 text-white px-8 py-4 rounded-full font-semibold text-lg hover:bg-purple-700 transition-all duration-300 transform hover:scale-105 shadow-lg">
            <span className="flex items-center gap-2">
